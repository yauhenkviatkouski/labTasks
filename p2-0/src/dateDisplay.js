export default class DateDisplay {
  parser(str, inputScheme) {
    const components = {};
    const defaultScheme = inputScheme || 'DDMMYYYY';
  ...
  print(components, outputScheme) {
    const defaultScheme = outputScheme || 'DD-MM-YYYY';
  ...
  fromNow(str, inputScheme) {
    ...
    const resStr = years ? `${years} years and ${days} days` : `${days} days`;
